...
$(function () {
    ...
    var valConfig = {
        ...
        submitHandler: function () {
            var ticketId = $("#ticketId").val();//场地票标识
            ...
            $.ajax({
                url:'/siteTicket/editSiteTicketBasicInfo.json',
                type:'POST', //GET
                data:reqParam,
                dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
                success:function(result){
                    if("000000" == result.code) {
                        layer.msg("编辑场地票基本信息成功~");
                        window.location.reload(true);
                    }
                },
                error:function(result){
                    layer.msg("编辑场地票基本信息失败~");
                }
            });
        }
    }
    $('#editSiteTicketBasicInfoForm').validate(valConfig);
});

/**
 * 更新子场馆列表
 */
function updateSubStadiumList(mainStadiumId, subStadiumId) {
    $.ajax({
