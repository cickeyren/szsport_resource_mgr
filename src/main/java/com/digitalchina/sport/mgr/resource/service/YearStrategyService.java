...
//        String fixDay = request.getParameter("fixDay");//固定日期
        if("0".equals(orderEffectiveType)) {
            //String orderEffectiveStartTime = DateUtil.formatDate(neeUtil.offsiteDate(new Date(System.currentTimeMillis()), Calendar.DAY_OF_MONTH,Integer.valueOf(fixDay)));
            //yearStrategyTicketModel.setOrderFixDay(fixDay);
            yearStrategyTicketModel.setOrderEffectiveStartTime(null);
            yearStrategyTicketModel.setOrderEffectiveEndTime(null);
        } else if("1".equals(orderEffectiveType)) {
            yearStrategyTicketModel.setOrderEffectiveStartTime(request.getParameter("orderEffectiveStartTime"));
            yearStrategyTicketModel.setOrderEffectiveEndTime(request.getParameter("orderEffectiveEndTime"));
        }
     ...
     */
    @Transactional
    public boolean doModifyYearStrategyTicket(YearStrategyTicketModel yearStrategyTicketModel,HttpServletRequest request) throws Exception {
       String strategyId = yearStrategyTicketModel.getId();
        //1删除相关联的子场馆列表
        yearStrategyDao.deleteYearStrategyStadiumRelationsListByYearStrategyId(strategyId);
        //2删除相关联的可用时间
        yearStrategyDao.deleteYearStrategyTicketCheckUseableTimeListByYearStrategyId(strategyId);
        //3删除相关联的不可日期
        yearStrategyDao.deleteTicketStrategyCommonCheckShieldTimeModelByYearStrategyId(strategyId);
...
//        String fixDay = request.getParameter("fixDay");//固定日期
        ...
        yearStdaiumList.add(stadiumModel);
        insertYearStrategyStadiumRelationsList(yearStdaiumList);
        yearStrategyDao.updateYearStrategyTicket(yearStrategyTicketModel);
        return true;
    }
